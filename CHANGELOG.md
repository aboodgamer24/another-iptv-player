# [0.7.5-1-BETA](https://github.com/aboodgamer24/c4tv-player/compare/v0.6.1-ALPHA...v0.7.5-BETA) (2026-04-29)

### Features

- **🚀 Ultra-Fast Startup**: Refactored the core loading engine into a two-phase system. The app UI now unlocks in <200ms, allowing users to browse categories while data populates in the background.
- **⚡ Immediate Content (0.6.1 Legacy Feel)**: Restored the "instant load" experience from version 0.6.1-ALPHA by implementing background bulk-loading for VOD and Series metadata.
- **📱 Zero-Flicker Mobile Navigation**: Optimized `MobileContentScreen` to skip loading states if data is already available in memory.

### Bug Fixes

- **Loading Logic**: Resolved a bug where the loading spinner would stay visible even after content was loaded in the grid view.
- **Navigation Shell**: Improved loading guards in `MainNavigationScreen` to prevent full-screen overlays when structural data (categories) is already present.
- **State Management**: Fixed an issue where switching categories would sometimes trigger redundant database queries.
