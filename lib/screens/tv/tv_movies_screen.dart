...
class _TvMoviesScreenState extends State<TvMoviesScreen> {
  ...
  Future<void> _selectCategory(String catId, XtreamCodeHomeController ctrl) async {
    ...
    if (catId == _kFavoritesCatId) {
      ...
      final favsData = await repo.getFavoritesByContentType(ContentType.vod);
      ...
      final List<ContentItem> items = itemsData
          .where((e) => e.contentType == ContentType.vod)
          .map((e) => ContentItem(
                e.streamId,
                e.title,
                e.imagePath ?? '',
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedCatId.isEmpty && realCats.isNotEmpty) {
      ...
      _currentItems = ctrl.getMoviesByCategory(_selectedCatId);
    }
          ...
          child: FocusScope(
            ...
            child: _loadingVirtual
                ...
                : _currentItems.isEmpty
                    ...
                    : TvContentGrid(
                        sectionKey: 'movies_$_selectedCatId',
                        items: _currentItems,
                        crossAxisCount: 5,
